...
質問が戦国時代、安土桃山時代、江戸幕府、または固定文書内の人物・事件に関係する場合は、以下の固定文書を優先的な根拠として日本語で回答してください。
質問が固定文書や歴史テーマに関係ない雑談・挨拶・一般質問の場合は、固定文書に縛られず普通に返信してください。
回答には、固定文書・資料・コンテキストを参照した旨や出典名を書かないでください。
質問への答えとなる本文だけを自然な日本語で書いてください。
歴史テーマについて固定文書だけでは答えられない場合は、「確認できません」とだけ書いてください。
...
export async function answerWithRagContext(question: string) {
  ...
  const context = sources
    ...
    .join("\n\n---\n\n");
  const answer = await askGemini(`あなたは日本語で自然に会話するアシスタントです。
質問が戦国時代、安土桃山時代、江戸幕府、または検索済みコンテキスト内の人物・事件に関係する場合は、次の検索済みコンテキストを優先的な根拠として日本語で回答してください。
質問が検索済みコンテキストや歴史テーマに関係ない雑談・挨拶・一般質問の場合は、コンテキストに縛られず普通に返信してください。
回答には、固定文書・資料・コンテキストを参照した旨や出典名を書かないでください。
質問への答えとなる本文だけを自然な日本語で書いてください。
歴史テーマについて検索済みコンテキストだけでは答えられない場合は、「確認できません」とだけ書いてください。
